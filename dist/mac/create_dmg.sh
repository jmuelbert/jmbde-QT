...
hdiutil create -size 200m "$TMP/$WORK_TEMPLATE" -type SPARSE -fs HFS+ -volname "jmbde"
  ...
  hdiutil attach "$TMP/$WORK_TEMPLATE" -mountpoint "$TMP/$VOLUME_TEMPLATE"
    ...
    cp images/jmbde-dmg-background.png "$TMP/$VOLUME_TEMPLATE/.background/"
    ...
    mkdir "$TMP/$VOLUME_TEMPLATE/jmbde.app"
    ...
    cp -fpR "$BUNDLE_PATH/jmbde.app/Contents" "$TMP/$VOLUME_TEMPLATE/jmbde.app"
...
rm "$BUNDLE_PATH/jmbde.dmg"
hdiutil convert "$TMP/$WORK_TEMPLATE" -format UDBZ -o "$BUNDLE_PATH/jmbde.dmg"
