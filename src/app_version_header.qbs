    ...
    Rule {
        ...
        prepare: {
            ...
            cmd.sourceCode = function() {
                ...
                content = content.replace(/(\n#define JMBDE_VERSION_DISPLAY_DEF) .+\n/, "$1 "
                        + product.moduleProperty("qtc", "qtcreator_display_version") + "\n");
                content = content.replace(/(\n#define JMBDE_VERSION) .+\n/, "$1 "
                        + product.moduleProperty("qtc", "qtcreator_version") + "\n");
                content = content.replace(/(\n#define JMBDE_VERSION_MAJOR) .+\n/, "$1 "
                        + product.moduleProperty("qtc", "JMBDE_version_major") + "\n");
                content = content.replace(/(\n#define JMBDE_VERSION_MINOR) .+\n/, "$1 "
                        + product.moduleProperty("qtc", "JMBDE_version_minor") + "\n");
                content = content.replace(/(\n#define JMBDE_VERSION_RELEASE) .+\n/, "$1 "
                        + product.moduleProperty("qtc", "JMBDE_version_release") + "\n");
