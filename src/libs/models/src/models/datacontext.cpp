...
void Model::DataContext::CreateConnection() {
  if (m_dbType == DBTypes::SQLITE) {
    ...
    qCInfo(jmbdemodelsLog) << "Open ODBC Database: " << this->m_Name
                           << " on host " << this->m_dbHostName;
    ...
    qCInfo(jmbdemodelsLog) << "Open PostgreSQL Database: " << this->m_Name
                           << " on host " << this->m_dbHostName;
...
void Model::DataContext::prepareDB() {
  ...
  if (!file.exists()) {
    qCCritical(jmbdemodelsLog)
        << "Fatal error on build database. The file '" << file.fileName()
  ...
  while (!file.atEnd()) {
    ...
    if (!line.isEmpty()) {
      if (!query.exec(line)) {
        qCCritical(jmbdemodelsLog)
            << "exec failed at line <" << line << "> : " << query.lastQuery()
            << " : " << query.lastError().text();
      }

    } else {
      qCCritical(jmbdemodelsLog) << "exec wrong at line <" << line
                                 << "> : " << query.lastError().text();
    }
  }
  file.close();
...
auto Model::DataContext::checkDBVersion(const QString &actualVersion,
  ...
  qCDebug(jmbdemodelsLog) << "CheckDBVersion, try getversion "
                          << m_db.lastError().text();
  ...
  if ((version == actualVersion) && (revision == actualRevision)) {
    qCDebug(jmbdemodelsLog)
        << "Check DBVersion: OK - Version:" << version << "." << revision << "."
        << build << " from " << lupdate.toString();

    retValue = true;
  } else {
    if (version < actualVersion) {
      qCCritical(jmbdemodelsLog)
          << "Check DBVersion: Database to old. Version : " << version
...
auto Model::DataContext::check_existence(const QString &tableName,
  ...
  if (query.exec()) {
    ...
    qCWarning(jmbdemodelsLog)
        << "CheckExistence(" << tableName << ", " << searchId << ", " << search
        << ") : " << query.lastError().text();
  }
...
auto Model::DataContext::insert(const QString &tableName,
                                const QVariantMap &insertData) -> bool {
  ...
  QString sqlQueryString =
      ...
      QString(fields.join(QStringLiteral(","))) + QLatin1String(") VALUES(") +
...
void Model::DataContext::renameDB(const QString &oldName,
  ...
  qCDebug(jmbdemodelsLog) << "Change Database Name: " << newName
                          << " to new Name " << newDBName;
...
auto Model::DataContext::getDatabase() -> QSqlDatabase { return m_db; }

auto Model::DataContext::initDb() -> QSqlError { return m_db.lastError(); }

void Model::DataContext::setDataBaseAccount(const QString &DBType,
