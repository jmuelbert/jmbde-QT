...
MainWindow::MainWindow(QWidget *parent)
  ...
  this->dataContext =
      new Model::DataContext(dynamic_cast<QObject *>(this), this->dataBaseName);
...
void MainWindow::initOutline() {
  ...
  QList<QString> subEntries = {tr("Employee"), tr("Function"), tr("Deparment"),
                               tr("Title")};
...
void MainWindow::readSettings() {
  ...
  settings.beginGroup(QLatin1String(Settings::Groups::WINDOWSSTATE));
  int window_x = settings
                     .value(QLatin1String(Settings::WindowState::WINDOW_X),
                            DEFAULT_WINDOW_X)
                     .toInt();
  int window_y = settings
                     .value(QLatin1String(Settings::WindowState::WINDOW_Y),
                            DEFAULT_WINDOW_Y)
                     .toInt();
  int window_width =
      settings
          .value(QLatin1String(Settings::WindowState::WINDOW_WIDTH),
                 DEFAULT_WINDOW_WIDTH)
          .toInt();
  int window_height =
      settings
          .value(QLatin1String(Settings::WindowState::WINDOW_HEIGHT),
...
void MainWindow::onClickedTreeView(const QModelIndex &index) {
  ...
  // Headers -> no action
  if ((selected == tr("Person")) || (selected == tr("Device")) ||
      (selected == tr("Communication")) || (selected == tr("Misc"))) {
    return;
  }
