...
DataModel::DataModel(QObject *parent) : QObject(parent) {
    ...
    const int dbType =
        settings.value(QLatin1Literal(Settings::Database::TYPE), SQLITE)
            .toInt();
    ...
    if (dbType == SQLITE) {
        ...
        targetFileAndPath; this->prepareDB();
        }
        else {
            this->openDB(this->name);
        }
...
void DataModel::prepareDB() const {
    ...
    if (!file.exists()) {
        QString log =
            QStringLiteral("Fatal error on build database. The file '");
...
bool DataModel::checkDBVersion() {
    ...
    if (version == QLatin1String(Database::Version::Version) &&
        ...
        if (version < QLatin1String(Database::Version::Version)) {
            qDebug() << "Database to old. Updata in progress! Version : "
                     << version
