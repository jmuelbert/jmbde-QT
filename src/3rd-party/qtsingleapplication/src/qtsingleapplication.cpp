...
QtSingleApplication::QtSingleApplication(const QString &appId, int &argc,
  ...
  // This shared memory holds a zero-terminated array of active (or crashed)
  // instances
  ...
  *pids = 0;
  pidPeer = new QtLocalPeer(
      this, appId + QLatin1Char('-') +
...
QtSingleApplication::~QtSingleApplication() {
  ...
  QtLockedFile lockfile(
      instancesLockFilename(QtLocalPeer::appSessionId(appId)));
  lockfile.open(QtLockedFile::ReadWrite);
  lockfile.lock(QtLockedFile::WriteLock);
...
QString QtSingleApplication::applicationId() const { return appId; }

void QtSingleApplication::setBlock(bool value) { block = value; }
...
void QtSingleApplication::setActivationWindow(QWidget *aw,
  ...
  if (activateOnMessage)
    connect(pidPeer, SIGNAL(messageReceived(QString, QObject *)), this,
            SLOT(activateWindow()));
  else
