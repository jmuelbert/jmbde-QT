...
QString QtLocalPeer::appSessionId(const QString &appId) {
  ...
  //### could do: two 16bit checksums over separate halves of id, for a 32bit
  // result - improved uniqueness probability. Every-other-char split would be
  // best.

  QString res =
      QLatin1String("qtsingleapplication-") + QString::number(idNum, 16);
#if defined(Q_OS_WIN)
...
QtLocalPeer::QtLocalPeer(QObject *parent, const QString &appId)
  ...
  if (id.isEmpty())
    id = QCoreApplication::
        applicationFilePath(); //### On win, check if this returns .../argv[0]
