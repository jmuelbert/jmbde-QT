...
Product {
    ...
    property string pathToSharedSources: FileInfo.joinPaths(path,
            FileInfo.relativePath(FileInfo.joinPaths('/', app.app_qbs_imports_path),
                                  FileInfo.joinPaths('/', app.app_shared_sources_path)))
    ...
    cpp.defines: app.generalDefines
    ...
    Group {
        fileTagsFilter: ["app.dev-module"]
        qbs.install: true
        qbs.installDir: app.app_qbs_modules_path + '/' + product.name
    }
