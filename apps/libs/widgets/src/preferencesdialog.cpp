...
void PreferencesDialog::writeSettings()
{
  ...
  if (dbConnectionString.length() < 2) { dbConnectionString = this->getUserDataDir(); }
...
void PreferencesDialog::readSettings()
{
  ...
  ui->lineEditDatabaseConnection->setText(
    settings.value(QLatin1String(Settings::Database::CONNECTION), this->getUserDataDir()).toString());
  ui->lineEditHostName->setText(
    settings.value(QLatin1String(Settings::Database::HOSTNAME), QLatin1String("localhost")).toString());
  ui->lineEditUserName->setText(
    settings.value(QLatin1String(Settings::Database::USERNAME), QLatin1String("jmbde")).toString());
  ui->lineEditPassword->setText(
    settings.value(QLatin1String(Settings::Database::PASSWORD), QLatin1String("jmbde")).toString());
}
...
void PreferencesDialog::on_pushButtonDBForceFileDialog_clicked()
{
  ...
  if (dbDir.length() > 2) { ui->lineEditDatabaseConnection->setText(dbDir); }
}
