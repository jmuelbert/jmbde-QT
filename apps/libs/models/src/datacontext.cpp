...
Model::DataContext::DataContext(QObject *parent)
  : QObject(parent), m_Name(QApplication::applicationName()), m_dbType(DBTypes::SQLITE)
{
...
Model::DataContext::DataContext(QObject *parent, const QString &name)
  : QObject(parent), m_Name(name.isEmpty() ? QApplication::applicationName() : name), m_dbType(DBTypes::SQLITE)
{
...
Model::DataContext::DataContext(QObject *parent,
  ...
  const int port)
  : QObject(parent), m_Name(name.isEmpty() ? QApplication::applicationName() : name), m_dbType(DBTypes::SQLITE),
    m_dbHostName(hostName), m_dbUserName(userName), m_dbPassWord(passWord), m_dbPort(port)
{
...
void Model::DataContext::prepareDB() const
{
  ...
  if (!file.exists()) {
    qCritical() << tr("Kritischer Fehler bei der Initialisierung der Datenbank.") << tr("Die Datei '")
                << file.fileName() << tr("' zum initialisieren der Datenbank ")
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    qCritical() << tr("Kritischer Fehler bei der Initialisierung der Datenbank.") << tr("Die Datei '")
                << file.fileName() << tr(" zum initialisieren der Datenbank ") << tr("kann nicht geöffnet werden.");
  }
  ...
  while (!file.atEnd()) {
    ...
    while (!hasText) {
      ...
      if (!cleanedLine.startsWith(QLatin1String("--")) && !cleanedLine.startsWith(QLatin1String("DROP"))
          && !cleanedLine.isEmpty()) {
        line += cleanedLine;
      }
      if (cleanedLine.endsWith(QLatin1String(";"))) { break; }
      if (cleanedLine.startsWith(QLatin1String("COMMIT"))) { hasText = true; }
    }
    if (!line.isEmpty()) {
      if (!query.exec(line)) {
        qCritical() << tr("Fehler beim Lesen der Datei zur Datenbank Erzeugung: ") << tr("Fehler in der Zeile <")
                    << line << "> : " << tr("Fehlermeldung: ") << query.lastQuery() << " : "
  ...
  /* const QString &actualBuild */) const -> bool
{
  ...
  if ((version == actualVersion) && (revision == actualRevision)) {
    qDebug() << tr("Prüfe Datenbank Version : ") << version << "." << revision << "." << build << tr(" von ")
             << lastUpdate.toString();

    retValue = true;
  } else {
    if (version < actualVersion) {
      qCritical() << tr("Prüfe Datenbank Version : ") << tr("Datenbank zu alt. Version : ") << version << "  <> "
...
auto Model::DataContext::checkExistence(const QString &tableName, const QString &searchId, const QString &search)
  -> bool
{
  auto queryStr =
    QString(QLatin1String("SELECT %1 FROM %2 WHERE %3 = \"%4\"")).arg(searchId, tableName, searchId, search);
...
auto Model::DataContext::insert(const QString &tableName, const QVariantMap &insertData) const -> bool
{
  ...
  for (qsizetype i = 0; i < totalFields; ++i) { strValues.append(QLatin1String("?")); }

  QString sqlQueryString = QLatin1String("INSERT INTO ") + tableName + QLatin1String(" (")
  ...
  int k = 0;
  for (const QVariant &value : values) { query.bindValue(k++, value); }
  return query.exec();
}

auto Model::DataContext::update(const QString &table,
...
void Model::DataContext::open(const QString &name)
{
  ...
  if (!this->m_db.isOpen()) {
    if (!this->m_db.isValid()) { this->m_db = QSqlDatabase::database(name); }
    if (!this->m_db.open()) {
...
void Model::DataContext::renameDB(const QString &newName)
{
  if (m_dbType == DBTypes::SQLITE) {
    ...
    qDebug() << tr("Ändere den Namen der Datenbank : ") << oldConnection << tr(" Neuer Name: ")
...
void Model::DataContext::setDatabaseConnection()
{
  ...
  if (!writeDir.exists()) { writeDir.mkpath(dbDataPath); }
