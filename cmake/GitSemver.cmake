...
#
# The idea was "taken" from: The stlink project (github.com/texane/stlink) See:
# https://github.com/texane/stlink/blob/master/LICENSE
#
# The following use cases work: - Dirty (sources changed) - No tags in repo (git
# hash is used) - Tags as: v1.0.0, 1.0.0

include(${CMAKE_CURRENT_LIST_DIR}/StringToSemver.cmake)

# Find the git program
function(find_git)
  find_program(git NAMES "git" # Linux, Windows
  )
  if(NOT git)
...
# Arguments: MAJOR: Variable name for the major version return value MINOR:
# Variable name for the minor version return value PATCH: Variable name for the
# patch version return value STRING: Variable name for the string version return
# value PATH: Path where the git command is executed
function(git_semver)
  set(options)
  set(oneValueArgs MAJOR MINOR PATCH STRING PATH)
  set(multiValueArgs)
  cmake_parse_arguments(git_semver "${options}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})
  ...
  if(NOT git_semver_PATH)
    message(
      WARNING
        "No PATH variable specified; Using CMAKE_CURRENT_LIST_DIR: (${CMAKE_CURRENT_LIST_DIR})"
    )
  ...
  if(git)
    ...
    semver(${${git_semver_STRING}} ${git_semver_MAJOR} ${git_semver_MINOR}
           ${git_semver_PATCH})
  endif()

  # Apply variables to parent scope
  set(${git_semver_MAJOR}
      ${${git_semver_MAJOR}}
      PARENT_SCOPE)
  set(${git_semver_MINOR}
      ${${git_semver_MINOR}}
      PARENT_SCOPE)
  set(${git_semver_PATCH}
      ${${git_semver_PATCH}}
      PARENT_SCOPE)
  set(${git_semver_STRING}
      ${${git_semver_STRING}}
